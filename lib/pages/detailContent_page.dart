...
class detailContent_page extends StatefulWidget {

  @override
  State<detailContent_page> createState() => _detailContent_pageState();
}

class _detailContent_pageState extends State<detailContent_page> {
  ...
  final String stikersDetailStikersUrl = '/api/stikers/detailStikers/';
  ...
  final String stikersDetailSimiliarSecondDetailUrl = '/api/stikers/detailSimiliarSecondDetail?search=';
  ...
  final _scrollDetailController = ScrollController();

  int allDataStikers = 0;
  int allDataStikersDetails = 0;
  ...
  Future<List> _stikersAllDetail() async {

    String? token = '';
    String? idStikers = '';
    token = await storage.read(key: 'token');
    idStikers = await storage.read(key: 'idStikers');
    ...
    try {
      // This is an open REST API endpoint for testing purposes
      final http.Response response = await http.get(Uri.parse(_baseUrl+stikersDetailStikersUrl+idStikers!), headers: {
        'Authorization': 'Bearer $token',
      });
  ...
  Future<List> _stikersAllOtherSimiliarSecondDetail() async {

    String? token = '';
    token = await storage.read(key: 'token');

    String? stikerNameDetail = '';
    stikerNameDetail = await storage.read(key: 'stikerNameDetail');
    ...
    try {
      // This is an open REST API endpoint for testing purposes
      final http.Response response = await http.get(Uri.parse(_baseUrl+stikersDetailSimiliarSecondDetailUrl+stikerNameDetail!), headers: {
        'Authorization': 'Bearer $token',
      });
    ...
    allDataStikersDetails = posts.length;
  ...
  void initState() {
    ...
    _stikersAllOtherSimiliarSecondDetail();
    _checkUser();
    _scrollController.addListener(listenScrollingController);
    _scrollDetailController.addListener(listenScrollingDetailController);
    myBanner.load();
    _initAd();
  ...
  int countOfData = 10;
  int countOfDataDetails = 10;

  void listenScrollingController()
  {
  ...
  void listenScrollingDetailController()
  {

    if(_scrollDetailController.position.pixels >= _scrollDetailController.position.maxScrollExtent)
    {
        if(countOfDataDetails <= allDataStikersDetails)
        {

          countOfDataDetails += 10;

          if(countOfDataDetails >= allDataStikersDetails)
          {
            countOfDataDetails = allDataStikersDetails;
          }
        ...
        body: SafeArea(
          child: Stack(
            children: [             
              ...
              IgnorePointer(
                ignoring: IgnoringSimiliarSearch,
                child: Opacity(
                  opacity: opacitysimiliarSearch,
                  child: Stack(
                    children: [
                      InkWell(
                        onTap: () {
                          setState(() {
                            opacitysimiliarSearch = 0;
                            IgnoringSimiliarSearch = true;
                          });
                        },
                        child: Container(
                      ...
                      Container(
                        ...
                        child: ListView(
                          ...
                          children: [
                            ...
                            SizedBox(height: 10,),
                            FutureBuilder(
                              future: _stikersAllOtherSimiliarSecondDetail(),
                              builder: (BuildContext ctx, AsyncSnapshot<List> snapshot) =>
                                snapshot.hasData
                                  ? 
                                  Container(
                                    child: GridView.builder(
                                      ...
                                      itemCount: countOfDataDetails >= snapshot.data!.length ? snapshot.data!.length : countOfDataDetails,
                                      itemBuilder: (BuildContext context, index) => 
                                      WeChooseForYouOther(
                                        snapshot,
                                        index
              ...
              InkWell(
                ...
                child: header_layout_arrow('', Colors.transparent, '', context),
              ),
              IgnorePointer(
  ...
  Widget detailStikers(AsyncSnapshot snapshot)
  {

    String imageWebp = snapshot.data![0]['imageWebp'];
    String stikerName = snapshot.data![0]['stikerName'];
    String amountDownload = snapshot.data![0]['amountDownload'].toString();
    String photoProfile = snapshot.data![0]['stikers_user']['photoProfile'];
    String username = snapshot.data![0]['stikers_user']['username'];
    String amountRating = snapshot.data![0]['stikers_category']['ratingAverage'] != null ? snapshot.data![0]['stikers_category']['ratingAverage'].toString() : '0';
    String amountComment = snapshot.data![0]['stikers_category']['stikers_comment'] != null ? snapshot.data![0]['stikers_category']['stikers_comment'].length.toString() : '0';
      ...
      children: [
        Stack(
          children: [
            Container(
              child: ClipRRect(
                child: Image.network('https://bukahuni.com/storage/stikersIdImages/$imageWebp', fit: BoxFit.fill, width: MediaQuery.of(context).size.width, height: MediaQuery.of(context).size.width,),
              ),
            ),
            Positioned(
              bottom: 10,
              ...
              child: InkWell(
                onTap: () {
                  setState(() {
                    opacitysimiliarSearch = 1;
                    IgnoringSimiliarSearch = false;
                  });
                },
                child: Container(
                  padding: EdgeInsets.symmetric(vertical: 11, horizontal: 8),
                  decoration: BoxDecoration(
                    color: whiteTransparent150Color,
                    borderRadius: BorderRadius.circular(25),
                    boxShadow: [
                      BoxShadow(
                        offset: Offset(0, 1),
                  ...
                  child: Image.asset('assets/images/ic_filter.png', height: 12,)
                ),
              ),
            ),
          ],
              ...
              Row(
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.start,
                    children: [
                      Icon(Icons.person_add_alt_sharp, size: 20, color: blackGray,),
                      SizedBox(width: 7,),
                      Container(
                        child: Text(
                          amountDownload, 
